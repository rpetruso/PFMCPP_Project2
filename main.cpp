...
void variableDeclarations()
{
    ...
    ignoreUnused(number, name, fish, letter, x, y, z, isTrue, gateOpen, computerOn, pi, a, b, cc, dd, ee); //passing each variable declared to the ignoreUnused() function
}
...
int main()
{
    ...
    //3)
    auto carSpeed = getCarSpeed(30, 60, 40);
    //4)
    auto quotient = divide(3.14f, 2.f);
    //5)
    auto product = multiply(48.0, 29.0);
    //6)
    auto flies = countFlies(23, 59);
    //7)
    auto average = getAverage(4.f, 5.f, 6.f);
    //8)
    auto bikeRented = isBikeRentedAndStoreInfo(1, 10);
    //9)
    auto numberOfSuits = getNumberOfSuits(15, 10);
    //10)
    auto numberOfShoes = getNumberOfShoes(5, 2);
    
    ignoreUnused(carRented, isCarRented, carSpeed, quotient, product, flies, average, bikeRented, numberOfSuits, numberOfShoes);
